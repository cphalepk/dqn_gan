...
class Options:
    def __init__(self):
        ...
        #Trickes
        self.replay_buffer_size = 200000 # The size of replay buffer; set it to size of your memory (.5M for 50G available memory)
        ...
        self.learning_frequency_GAN = 16
        self.reset_frequency_GAN = self.replay_buffer_size/4
        self.batch_size_GAN = 128
...
for i in range(opt.num_episode):
    ...
    while not done:
        ...
        if frame_counter > opt.replay_start_size:
            if frame_counter % opt.learning_frequency_GAN == 0:
                replay_memory.sample_GAN(opt.batch_size_GAN, batch_frame)
                StepGAN(batch_frame, D, D_trainer, G, G_trainer, False, True)
